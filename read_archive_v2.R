...
read.weatherlink <- function(filename, station_name = 'Метеостанция', 
                             graph_title = 'График измеренных метеовеличин', 
                             date_breaks = '1 day', 
                             minor_date_breaks = '1 hour',
                             xls = F, baro = F, amdate = F, long = F){
  # считываем данные
...
df <- read.weatherlink(filename)
df <- df %>% 
  pivot_longer(!c(DateTime, station_name), names_to = 'var', values_to = 'val')
